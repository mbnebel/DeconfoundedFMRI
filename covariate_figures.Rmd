...
nested_gams <- aim1Nest %>%
  mutate(model = map(data, ~mgcv::gam(1-delta~s(value, k=-1)+dummySex+s(SES.Family_centered, k=-1), data = na.omit(.x), 
...
nested_gams <- nested_gams %>% 
           mutate(LB = map(data, ~round(min(na.omit(.x$value)))),
                  ...
                  fit.data = map(range, ~data.frame(value = .x, dummySex = rep(0, length=length(.x)), SES.Family_centered = rep(0, length=length(.x)))),
...
nested_mw_less <- aim1MW %>% 
  ...
  #calculate Z using qnorm
  mutate(Za = map(p.value, ~qnorm(.x/2))) %>% 
  #calculate effect size from Z
  mutate(r = map2(Za, data, ~(abs(.x)/sqrt(length(.y)))))
