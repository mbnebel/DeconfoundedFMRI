...
```{r define-split-violin, tidy=TRUE}
...
#### Visualize missingness in covariate data:

```{r vis-missing-covariates}
...
pdf('./CovariatesAndRS-fMRIUsability/vis_miss.pdf')
visdat::vis_miss(dat.forcompletepredictors, sort_miss = TRUE)
dev.off()
```
...
```{r exclusion-flow, echo=FALSE, dev='png', results="asis"}
...
graph %>% export_svg() %>% 
  charToRaw() %>% 
  rsvg_png("./CovariatesAndRS-fMRIUsability/exclusionFlowChart.png", width = 2700, height =1200)
...
png("./CovariatesAndRS-fMRIUsability/fig_propExcludedDx_cc.png",width=3,height=2.5,units="in",res=300)
dx_proportions
invisible(dev.off())
...
```{r prop-excluded-numbers, results="asis"}
...
```{r gam-theme, message=FALSE, warning=FALSE}
gam_theme = theme(
  axis.title.x=element_text(size=12),
...
```{r density-theme, echo=F, message=FALSE, warning=FALSE}
den_theme = theme(
  axis.title.x=element_text(size=12),
  axis.title.y=element_text(size=12),
  axis.text.x=element_text(size=8),
...
```{r combine-gam-densities, fig.width=10, fig.height=6}
...
png("./CovariatesAndRS-fMRIUsability/fig_probExclusion_allGAM_TD_ASD_cc.png",width=10,height=6,units="in",res=300)
cowplot::plot_grid(p_legend, top_row, NULL, bottom_row, NULL, hist_legend, nrow=6, rel_heights=c(.1, 1, -.01, .5, -.07, .1))
dev.off()

#png("~/Dropbox/Apps/Overleaf/MotionSelectionBias_rsfMRI/Figures/fig_probExclusion_allGAM_TD_ASD_cc.png",width=10,height=6,units="in",res=300)
...
ezknit("ReviewerResponse/forReviewers_probEx_withSexSESRace.Rmd", out_dir = "ReviewerResponse/probEx_withSexSESRace")
...
ezknit("ReviewerResponse/forReviewers_meanFD_covariates.Rmd", out_dir = "CovariatesAndRS-fMRIUsability/mean_FD_covariates")
...
ezspin("FisherTestsRaceAndSex.R", wd = "ReviewerResponse", out_dir = "meanFD-race-sex")
...
```{r combine-violins, echo=F, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 4}
...
png("./CovariatesAndRS-fMRIUsability/fig_violins_wide_cc.png",width=8,height=4,units="in",res=300)
...
```{r combine-pValue-histograms, fig.width=8, fig.height=3}
...
png("./CovariatesAndRS-fMRIUsability/fig_hist_rfc_cc.png",width=8,height=3,units="in",res=300)
...
```{r delta-framewise-displacement}
...
```{r combine-FD, echo=F, message=FALSE, warning=FALSE, fig.width = 5, fig.height = 3}
png("./CovariatesAndRS-fMRIUsability/fd_metrics_2levels.png",width=5,height=2.5,units="in",res=300)
