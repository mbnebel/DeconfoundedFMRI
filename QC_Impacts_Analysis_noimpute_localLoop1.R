# Modified BRisk's QC_Impacts_Analysis_singleseed_noimpute_cb.R to run locally
...
temp = dat2[,c('PrimaryDiagnosis','MeanFramewiseDisplacement.KKI','MaxFramewiseDisplacement.KKI',"FramesWithFDLessThanOrEqualTo250microns",
               "Sex","Race2","SES.Family","ic1.ic2")]
...
gn.variables = c('KKI_criteria','PrimaryDiagnosis','ADHD_Secondary','AgeAtScan','handedness','CurrentlyOnStimulants',
                 'PANESS.TotalOverflowNotAccountingForAge','WISC.GAI','DuPaulHome.InattentionRaw','DuPaulHome.HyperactivityRaw','ADOS.Comparable.Total')
...
gam.prop.model = mgcv::gam(Delta.KKI~PrimaryDiagnosisNone+ADHD_Secondary+handednessMixed+handednessRight+CurrentlyOnStimulants+
                             s(AgeAtScan)+s(DuPaulHome.InattentionRaw)+s(DuPaulHome.HyperactivityRaw)+s(WISC.GAI)+
                             s(PANESS.TotalOverflowNotAccountingForAge)+s(ADOS.Comparable.Total),method='REML',family=binomial,data=gn.xmat)
propensities.gam=predict(gam.prop.model,type='response')

for(seed in seeds){
  set.seed(seed, "L'Ecuyer-CMRG")
  
(propensity.KKI = mcSuperLearner(Y = Delta.KKI, X = gn.xmat, family=binomial(link='logit'),
                                 SL.library = my.SL.libs.gn, cvControl = list(V = 10), method='method.CC_nloglik')) # 10-fold CV
...
if (save.input.data) {
  save(file=paste0('./Data/noImputation/DataWithPropensities_seed',seed,'.RData'),dat3)
}

results.df$seed = seed
save(file=paste0('./Results/noImputation/ic30_pc85_glm_gam_drtmle_seed',seed,'.RData'), results.df)
