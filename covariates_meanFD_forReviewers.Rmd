...
png("./CovariatesAndRS-fMRIUsability/fig_meanFD_allGAM_TD_ASD_cc_none.png",width=10,height=5,units="in",res=200)
cowplot::plot_grid(top_row, NULL, 
                   second_row, NULL, 
                   hist_legend, nrow=5, rel_heights=c( 1, -.01, 
                                                       .5, -.05, .1))
dev.off()
...
ddata <- nested_gams %>% 
  filter(variable=="ADOS") %>% 
  select("variable", "data", "LB.y", "UB.y") %>% 
...
bottom_row <- cowplot::plot_grid(d_ados_l, d_srs_l, d_in_l, d_hi_l, d_mo_l, d_age_l, d_gai_l, ncol=7, 
                                 rel_widths=c(1.18/7, .97/6, .97/6, .97/6, .97/6, .97/6, .97/6))

png("./CovariatesAndRS-fMRIUsability/fig_meanFD_allGAM_TD_ASD_cc_2levels.png",width=11,height=10,units="in",res=200)
cowplot::plot_grid(top_row, NULL, 
                   second_row, NULL, 
                   third_row, NULL, 
                   bottom_row, NULL, 
                   hist_legend, nrow=9, rel_heights=c( 1, -.05, 
                                                        .5, -.05, 
                                                        1, -.05, 
                                                       .5, -.07, .1))
dev.off()
...
nested_gams_table[, c(1:2,6:10, 19)]

```



**Figure. mean FD changes with phenotype and age.** Univariate analysis of mean framewise displacement (FD) as a function of participant characteristics. From left to right: Autism Diagnostic Observation Schedule (ADOS), social responsiveness scale (SRS) scores, inattentive symptoms, hyperactive/impulsive symptoms, total motor overflow, age, general ability index (GAI), and socioeconomic status (SES) using all participants with usable and unusable data (black lines, all FDR-adjusted p<`r toString(round(max(nested_gams$p.fdr[nested_gams_table$Motion.Exclusion.Level=="None"]), digits=2))`),  participants passing lenient (light blue lines, all FDR-adjusted p<`r toString(round(max(nested_gams$p.fdr[nested_gams_table$Motion.Exclusion.Level=="Lenient"]), digits=2))`), and strict (red lines) motion quality control (all FDR-adjusted p<`r toString(round(max(nested_gams$p.fdr[nested_gams_table$Motion.Exclusion.Level=="Strict"]), digits = 2))`). Variable distributions for each diagnosis group (included and excluded scans) are displayed across the bottom panel (TD=typically developing, green; ASD=autism spectrum disorder, yellow).
